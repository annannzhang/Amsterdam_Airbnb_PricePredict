...
details.clean <-
  details.sf %>%
  dplyr::select(id, property_type, room_type, accommodates, bathrooms, bedrooms, beds, bed_type, minimum_nights, number_of_reviews, review_scores_rating, instant_bookable, availability_30, amenities.number, dist.metro, price)
...
reg.1 <- 
  lm(price ~ ., data = as.data.frame(details.clean) %>% 
  dplyr::select(-geometry, -beds))
